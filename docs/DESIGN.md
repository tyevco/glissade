# glide_and_slide — Architecture & Design
...
| | **glide_and_slide** | Motion Canvas | Remotion | Theatre.js | GSAP | Rive |
|---|---|---|---|---|---|---|
...
- **Shaping:** delegated to the canvas implementation (`fillText`/`measureText`) — browser Skia and @napi-rs/canvas both shape via HarfBuzz-backed Skia text. We do not ship a shaper in v1; the IR's string-carrying `fillText` op reflects that, with a glyph-run op reserved for a future harfbuzzjs path.
...
- **Measurement drift (decided):** browser and CLI Skia/HarfBuzz versions differ, so sub-pixel `measureText` deltas are possible — and since the same measurer feeds Yoga, drift moves *entire layouts*, not just line breaks. Decision: accept + document per the per-path determinism contract (§5.5), and **quantize all layout-feeding measurements** (line breaking *and* Yoga measure functions) to a quantum calibrated empirically against observed cross-version deltas before pinning (1/64 px is likely too fine for FreeType-vs-DirectWrite advance drift). harfbuzzjs-WASM identical-everywhere shaping remains the escape hatch if real-world breakage appears. Consequence for CI: byte-exact cross-*path* assertions involving text or text-sized layout are off the table; that's what the SSIM suite is for (§7.3).
...
**Why:** frame-accurate (timestamps set explicitly from frame index, never sampled from a clock), faster-than-realtime (encode as fast as the encoder drains, throttled on `encodeQueueSize` backpressure), hardware-accelerated, and in Chrome the canvas→VideoFrame→encoder path can stay zero-copy on the GPU. Mediabunny is the clear muxer choice: pure-TS, MPL-2.0, supersedes the deprecated `mp4-muxer`/`webm-muxer`, microsecond-accurate timestamps, streaming output with backpressure — and Remotion itself adopted it internally.
...
Consolidated surviving open questions. Everything else raised in earlier drafts has been promoted to a decision in place (track addressing §2.2; overlapping tracks §2.6; transition blending §4.7; measurement drift §3.6; write-back §6.2; WebCodecs milestone ordering §7.5; filter-set enumeration §3.4/M2; Rive marketing posture §1.5).

1. **Export segment join strategy (§5.6).** *Options:* (a) per-shard encodes joined by concat demuxer — requires GOP-aligned boundaries (forced keyframe at each range start) and identical encoder settings; low disk I/O; (b) shards emit lossless intermediates (FFV1/rawvideo) with a single final encode — simpler, bit-exact final stream, heavy disk I/O. Note: frame-level determinism is satisfied by both; this is pure engineering. *Recommendation:* (a) as default, `--lossless-intermediate` as the escape hatch; decide before the farm story is documented.
2. **Sidecar granularity (§6.2).** *Options:* (a) one `*.edits.json` per scene module — clean git-blame/review, local merges, but cross-scene shared state (labels, audio markers, render presets, panel layouts) has no home; (b) one per project; (c) per-scene sidecars plus a small project-level `gliss.project.json` for shared editor state. *Recommendation:* (c), with the project file's scope (markers? panel layouts? render presets?) decided before the studio MVP — keeping it minimal (shared markers + render presets only) is the safest start.
3. **`<gs-player>` controls inventory and budget (§4.3, §4.4).** Direction is locked (minimal CSS-part-themable controls behind the `controls` attribute, tree-shaken when absent; ≤ 5 kB sub-budget in `@gliss/element`). *Open:* the concrete control inventory (play/pause + scrubber only? time readout? loop toggle? volume once audio lands?) and its accessibility spec. *Recommendation:* play/pause, scrubber, time readout; everything else via CSS parts and slots — but this needs a real design pass before M1's demo page hardens into API.
4. **Video scope in v1 (§3.8, §5.4, M4).** *Options:* (a) keep `Video` in v1 with its own milestone (M4 as drafted) — the MC community expects media support and audio-without-video is an odd story; (b) cut to v1.x — the single biggest available de-scope: two decoder pipelines, backward-scrub machinery, and parity testing are a multi-month workstream. *Recommendation:* (a), with M4 explicitly designated the first de-scope candidate if v1 slips; the `VideoFrameSource` seam means cutting it removes a milestone, not an architecture.
5. **Deterministic transcendental math (§2.5, §5.5).** ECMAScript leaves `Math.exp/sin/pow` precision unspecified, so cross-engine DisplayList hashes differ in ULPs. *Options:* (a) scope bit-exactness to "same engine + pinned deps" (current contract) and document it; (b) ship fdlibm-style polynomial implementations for every easing/spring (lockstep-game-engine style) — cross-engine hash-identical, at a perf and maintenance cost. *Recommendation:* (a) for v1 — no identified user need requires cross-engine hashing; revisit (b) only if browser-authored documents must hash-match CLI renders (e.g., content-addressed render caching across the seam).
6. **Rive table verification (§1.5).** A pre-publish fact check, not a design question: the Rive row (proprietary editor, open-source runtimes, state-machine model) reflects general knowledge and must be verified against Rive's current licensing/editor model before this document or the positioning table is published. *Recommendation:* assign with the M6 launch checklist.