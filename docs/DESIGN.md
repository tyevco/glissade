...
**Persistent whole-frame raster cache — the `.gscache` (0.12, `@glissade/cli`).** The in-memory group LRU above spans groups WITHIN one process; the disk cache spans WHOLE FRAMES ACROSS runs and shards, so a one-line edit doesn't re-rasterize every blur-heavy frame on every run. It is opt-in — `gs render --cache [<dir>] [--cache-max-size <bytes|2GB>]` / `render({ cache: { dir, mode: 'read-write'|'read-only'|'off' } })` — and `mode:'off'` (the **default**) bypasses it entirely, preserving the exact current equality baseline (no behavior change unless opted in). Scope is LOCKED to **whole-frame granularity** (per-group disk tiling is DEFERRED to 0.13): the key is over the ENTIRE frame's DisplayList, so a hit is the "this frame's content is byte-identical to a prior render" case — byte-safe by construction.

The cache's single failure mode is SILENT corruption — a stale frame served from an INCOMPLETE key — so the key MUST fold EVERY byte-affecting input: (1) the frame's DisplayList-snapshot bytes via the shipped byte-preserving `serializeDisplayList` (resources inlined to content; the post-evaluate DisplayList already encodes resolved device transforms, so a whole-frame snapshot captures geometry/paint/transform); (2) the glissade **VERSION** — any Raster2D-composite or Skia-toolchain change ships under a new lockstep `0.x` version, making bump-on-version the cheapest CORRECT invalidation (text/font tiles are thereby scoped to the pinned toolchain — cross-toolchain reuse is OUT, consistent with §3.6 text being SSIM-only cross-path); (3) the **BackendCaps id** (filters/shaders/maxTextureSize). `version` and `capsId` have no source inside `scene`, so they are INJECTED via a `CacheKeyContext` from the CLI, never read from disk. `key = sha256(serializeDisplayList(frame) ++ version ++ capsId)`.

**HIT == MISS by construction:** the hit path reuses the SAME composite/encode routine as a miss — it loads the stored RGBA back into the backend (`SkiaBackend.putPixels`) and hands it to the IDENTICAL `encodePng` / ffmpeg downstream a miss takes, so a hit is byte-identical to a cold render (the `getImageData → putImageData → toBuffer` round-trip is verified byte-exact). **Storage** is raw-RGBA + zlib (Node `zlib`), one atomically-written (`temp + rename`) file per frame keyed by the hash — NO ffmpeg/FFV1. Because whole-frame RGBA is ~MBs/frame (tens of GB/episode), a **size-capped LRU ships from day one** (access-time ordered via filesystem mtime, default 2 GB), evicting oldest entries past the cap. Shards SHARE one `.gscache` dir; content-addressing + atomic writes make a delete/regenerate race between shards benign.

**The verify gate (mandatory).** `gs cache verify <scene>` renders read-only (serving cache hits) and a cache-OFF pass, asserting the `encodePng` bytes are EQUAL frame-for-frame — extending the §5.5 committed-RGBA-byte-hash machinery across the disk boundary. A sampled fraction is allowed for speed and the sample is LOGGED (no silent caps). A companion **NEGATIVE gate** (a test) injects a deliberately INCOMPLETE key (dropping the version / a transform-bearing component) and asserts the verify FAILS — proving the gate catches the only failure mode. **Honesty:** the cache wins REPEATED renders + the UNCHANGED-PREFIX of a single-segment edit; a full re-narrate shifts every frame's timing → every frame's DisplayList changes → every frame MISSES.
...
**A second reserved backend slot — DOM/SVG (0.20 design probe).** Beside the WebGPU effect layer, a DOM/SVG *rendering* backend (the §1.5 "out of scope for v1" item) is the other non-canvas consumer the IR must keep cheap to add. Its seams — supplying the DOM tree's required *stable node identity* without taxing the identity-less DisplayList (resolved **out-of-band**, a parallel id stream keyed by the already-stable emit walk order, so the §3.3 DrawCommands and all goldens stay byte-identical), the `mount({ backend })` injection point, and its explicitly **preview / non-parity** status (no canvas ⇒ neither byte-exact nor SSIM parity applies) — are resolved in the design memo **[`docs/design/dom-backend.md`](./design/dom-backend.md)**, with a throwaway read-only spike (`packages/scene/spike/`) proving the out-of-band-identity claim. It is a design-probe, not a v1 commitment.
...
**0.20 base-embed budget review (pre.0).** The base embed crept to 38.79/39 — full, blocking every embed-touching 0.20 feature. The standing posture (evaluate/render-path code spends real base budget; studio/dev/diagnostic/optional-helper code lives on a tree-shakeable subpath, mirroring the §3.2 `scene/layout` and §3.6 `core/font-ingest` splits) recovered the headroom by relocating three non-render surfaces off the base barrels: **`@glissade/core/sidecar`** (the §6.2 editor sidecar — studio-only), **`@glissade/scene/diagnostics`** (the §3.3 diff/snapshot substrate + `auditCacheCold` — DEV/CLI only, debug-only; the byte-preserving §3.5 `collapseReplacer` is the one render-path member and stays on the base index in its own module), and **`@glissade/scene/motion`** (the §3 motion-path follow helper — a user-facing opt-in, re-exported onto the `@glissade/browser` IIFE so `window.glissade.motionPath` survives). (A later ai-training finding split the PRODUCTION token-highlight render component `tokenHighlight` — visible UI in real episodes, NOT a debug surface — back out of `/diagnostics` onto its OWN production subpath **`@glissade/scene/tokens`** (npm-subpath-only — re-exporting it onto the IIFE measured +1.16 kB gz, busting the 48 kB convenience-bundle ceiling, so it is not on `window.glissade.*`); `/diagnostics` is debug-only.) Base embed: 38.79 → ~34.93 kB gz; the 39 ceiling is held, the recovered headroom is the 0.20 feature budget. Per-subpath metafile-exclusion guards in `scripts/check-size.mjs` assert each surface stays off the base graph so a stray re-import can't silently re-bloat the embed. The CI-vs-local measurement delta (CI read the base embed ~0.16 kB heavier and red-failed a 0.19.1 release) was an esbuild caret-pin patch float between environments — `esbuild`/`tsdown` are now pinned EXACT so the measured gz is identical local and CI.
...
**`gs verify-determinism` — the cross-shard/cross-machine divergence LOCATOR.** Where `gs diff` (§3.3) explains a single-frame mismatch against a committed `.dl.json`, `verify-determinism` asserts the *frame-level* determinism contract a sharded/distributed render leans on, and bisects the first divergence to a `(frame, node, op)`. It emits a **`frames.manifest`** — per-frame `sha256` of the **raw RGBA** from `backend.readPixels()` (NOT `encodePng`, sidestepping any PNG-encoder nondeterminism; `node:crypto` sha256, the §5.3/byte-hash committed-hash precedent — no new hash dep) plus per-node DisplayList sub-hashes (reusing the shipped `serializeDisplayList` collapse serializer). It evaluates under the SAME `withDeterminismGuards('throw')` as `gs render`, so a clock/random/timer call in scene code throws *during verification* — the verifier never perturbs the contract it checks. `--shards N` diffs a linear render against an N-shard render of the same range (each shard re-runs the module from scratch, exactly as a `gs render --workers` child does); `--against <manifest>` diffs a committed / other-machine manifest; `--bisect` drills the first divergence by replaying each side to the divergent frame and feeding the node's isolated emit to `diffDisplayLists` + `formatDisplayDiff` (§3.3). **The full-frame RGBA hash is authoritative; the per-node sub-hashes only LOCALIZE** — a node whose `emit()` depends on the inherited CTM sub-hashes differently in isolation than in context, so a sub-hash is a locator, never the byte authority, and a Group (its emit recurses) defers to a divergent leaf (mirroring `auditCacheCold`'s groupFallback). **HONEST SCOPE (§5.5 item 6):** byte-equality is **Skia↔Skia only** (cross-machine / cross-shard, pinned toolchain). The manifest stamps its `backend`, and an `--against` cross-backend byte-compare is **REJECTED** with a clear error — comparing a Canvas2D manifest by byte-hash is a category error, since browser↔Skia is perceptual (SSIM, §3.4/§7.3) parity, never byte-identity. The `frames.manifest` is a committed-baseline candidate but is *not* a stability-promised interchange schema like the §7.4 trio; it carries a `manifestVersion` for forward-compat only.
...
| **v2 themes** | Event-driven transitions/state machines on the signal layer; pointer/audio drivers; WebGPU effect backend (outside the determinism guarantee); **DOM/SVG preview backend** (design probe: [`docs/design/dom-backend.md`](./design/dom-backend.md) — out-of-band node identity, `mount({ backend })` injection, preview/non-parity); Lottie import/export | — | GPU nondeterminism (contained by design) |
