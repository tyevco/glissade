...
- a8b3ed3: critique: add the `OUT_OF_BOUNDS` diagnostic + the `containBounds` option — the INVERSE of `CAPTION_COLLISION`.
...
- 1073749: **0.64.0 — `safeAreas` + `CAPTION_COLLISION`: reserved caption-band verification.** A reserved region (e.g. the bottom caption band) is a _fill-zone_ for its owner (the caption) and a _forbidden-zone_ for everything else. One `SafeArea` primitive + a band-aware check closes three real-content needs — and it is a CRITIQUE concern only, never a render input, so every rendered byte is unchanged.
...
- b1e0658: **0.63.1 — a per-call legibility floor + discoverable options + the loop guide in-package.** Three additive, default-preserving fast-follows from the 0.63 author-loop capstone.
...
- **0.63 the closed-author-loop capstone** — composes the whole verification suite (validateScene + critique + parity + diff + certify) into the author→render→critique→self-fix cycle. The framework owns the _verdict_; the agent owns the _fix_.
...
- 969585a: **0.62 `gs render --certify`** — the determinism certificate + a content-addressed render cache. Determinism-as-a-moat, made a product: a render is only cacheable-by-content if it's byte-deterministic, which glissade is (b4e6060006 held 0.20→0.61).
...
- 9959e33: **0.61 `gs parity --semantic` + `diff(a,b)` + export-fidelity** — the "what's lost / what changed" pair that completes the agent-perception suite (validateScene + critique + parity + diff), all sharing one diagnostic schema, all byte-deterministic.
...
- 9e04239: 0.61 pre.1 — parity attribution correctness + diff fail-loud (canary verify fixes):
...
- fa460ad: **0.60 `critique(scene, timeline)`** — machine-readable RENDERED render diagnostics, the loop-closer that lets an agent SEE and self-fix its own render. The rendered-geometric counterpart to `validateScene()`'s static-structural checks (Era A).
...
- c7ba720: 0.60 pre.1 — canary verify fixes:
...
- 600eec2: **0.59 "fail-loud ground floor"** — close the perception gap so an agent can validate, inspect, and self-fix its own scenes. The ground under `critique()` (0.60).
  ...
  - **`resolveAt(scene, target, t)`** (on `@glissade/scene/diagnostics`) — read a node's _resolved_ property value at time `t`. A bound prop returns its real bound value (not the static default that inspection otherwise shows), closing the "looked broken but the binding was fine" trap. Load-bearing for `critique()`.

  - **Bound-prop inspection** — `instanceProps(node)` reports which props are _currently bound on this instance_, so an agent inspecting its work knows when a static read is untrustworthy and must use `resolveAt`.
...
- 6191235: 0.59 pre.1 — canary verify fixes:
...
- ad56b2f: Lottie export: MotionBlur and Echo now WARN (never-silent) that their render-only effect isn't exported.
...
- 2293906: 0.58 papercuts: gradient smooth/gaussian bloom export + typeOn `cursorFill`.
...
- b89a65a: Particles `drift` preset: safe-area default + `safeBottom` clamp (0.57.1 fast-follow).
...
- a537816: Particles/Emitters — `particles(spec)` + `drift`/`sparks`/`dispense` presets on `@glissade/scene/motion`.
...
- dd9295f: Kinetic type presets — one-call animated typography (`@glissade/scene/type`).
