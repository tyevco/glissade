...
import { estimatingMeasurer, isEstimatingMeasurer } from '../src/text.js';
...
describe('splitText() measurer-fail-loud (nUagJ5351H7t)', () => {
  ...
  it('THROWS by default when splitText would fall back to the per-character estimate (no measurer, no scene)', () => {
    // a bare Text props (no scene injected, no { measurer }, no setDefaultMeasurer)
    // resolves to estimatingMeasurer → fail loud, with a message that names the fix.
    let caught: unknown;
    try {
    ...
    expect((caught as Error).message).toMatch(/splitText: text geometry needs a real measurer/);
    expect((caught as Error).message).toMatch(/\{ estimate: true \}/);
    expect((caught as Error).message).toMatch(/length×0\.52/);
  });

  it('{ estimate: true } is the SOLE opt-out — degrades to the estimate, no throw', () => {
    expect(() => splitText({ id: 'w2', text: 'again', fontSize: 40 }, { estimate: true })).not.toThrow();
  });

  it('does NOT throw when a real { measurer } is passed (the exact-layout path)', () => {
    expect(() => splitText({ id: 'ok', text: 'split the text', fontSize: 40 }, { measurer: fixed })).not.toThrow();
  });
});
