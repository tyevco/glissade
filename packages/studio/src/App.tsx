...
export function App() {
  ...
  const extractEdits = useCallback(
    (target: string) => {
      ...
      if (!tr || !sidecar) return;
      void navigator.clipboard?.writeText(trackSource(tr as Track));
      undoStack.current.push(JSON.parse(JSON.stringify(sidecar)) as SidecarDoc);
      const next = deleteSidecarTrack(sidecar, 'main', target);
      const main = next.timelines['main'];
      const empty = !main || (Object.keys(main.tracks).length === 0 && !main.labels);
      const restored = empty ? null : next;
      setSidecar(restored);
      setPreview(null);
