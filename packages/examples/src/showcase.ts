...
import cameraRig from './scenes/golden-camera.js';
import cameraFrame from './scenes/golden-camera-frame.js';
import particlesScene from './scenes/golden-particles.js';
...
import layoutOverflow from './scenes/golden-layout-overflow.js';
...
const gallery: Record<string, { mod: SceneModule; blurb: string }> = {
  ...
  camera: { mod: cameraRig, blurb: 'Camera rig: a push-in (cam/zoom ramps while cam/center pans) over a 2-depth layer stack — the far backdrop parallaxes (pans less) while the focal plane leads — under a fixed-seed whole-frame shake. The camera transforms the WORLD as a parent transform, so an anchor:\'left\' bar stays anchored (no double-shift), and the caption is a SIBLING outside the rig so it stays pinned. Byte-compared on Skia.' },
  'camera-frame': { mod: cameraFrame, blurb: 'Camera NODE-FRAMING: centerOn tracks the hero node by id in WORLD space — the camera pans to follow it as it drifts (a far dot grid parallaxes, panning less). A clear reserved band nudges the tall hero\'s bounds up so its bottom rides clear of the caption zone, where a pinned caption sibling lives. The resolved focal is readable via resolveAt(scene,\'cam/resolvedCenter\',t). Byte-compared on Skia.' },
  ...
  particles: { mod: particlesScene, blurb: 'Particles: a seeded, BAKED emitter composing each() (fixed slot nodes) + bake() (seeded physics → ordinary tracks). A sparks radial burst (short-life dots shrinking + fading under gravity) beside an ambient drift field of low-opacity motes floating up. count is the max-CONCURRENT ring-buffer pool (not total emitted), opacity-gated so every slot is a real exportable layer — faithful by construction, byte-stable on Skia.' },
  ...
  layoutOverflow: { mod: layoutOverflow, blurb: 'The LAYOUT_OVERFLOW showcase: a real Row of three cells where the middle cell’s fat stroke ink bleeds out of the flex slot Yoga sized from its stroke-free intrinsic box — the content-bigger-than-its-cell defect critique() reports automatically over Layout nodes, shown as pixels.' },
