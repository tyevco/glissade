...
const BUDGETS = {
  ...
  'core/expr': 6, // 0.40 Expr: the deterministic math-formula evaluator (tokenizer + precedence-climbing parser → closure + a pure Math/constant whitelist + seeded rand) behind exprTrack. Tree-shakeable subpath OFF the base embed — a ~1.4 kB parser that must not ride every scene's render path; the base sampler carries only a tiny compiler-register seam (sampleTrack's tr.expr branch + setExprCompiler), and importing @glissade/core/expr activates it. Standalone bundle inlines the same-package track/valueTypes helpers exprTrack builds through (the @glissade/* external only catches CROSS-package deps), so the measured ~2.9 kB is mostly that shared construction path, not the evaluator's own surface; base core stays expr-free (asserted by the metafile guard below).
