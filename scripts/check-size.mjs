...
const BUDGETS = {
  ...
  'scene/layout': 56, // §3.2: Yoga (wasm-base64 + bindings) ships ONLY in this separate entry, never the base scene bundle. 0.20: this entry now re-exports the Yoga-free ctors from layoutCtors.ts PLUS loadYogaLayoutEngine from layoutEngineYoga.ts (the dynamic `import('yoga-layout/load')`), so it STILL inlines Yoga in the standalone bundle. 55->56 (measurer-fail-loud, card nUagJ5351H7t): Layout.computedSize + the Text-geometry getters this entry inlines now route through the shared resolveMeasurer/MeasurerRequiredError fail-loud chokepoint (the silent length×0.52 estimate is now a hard error unless { estimate: true } opts in), ~0.17 kB gz over the Yoga-dominated 55.00. Off-base (the SACRED base embed is UNCHANGED and under 38.83); a small correctness-floor bump on the Yoga-heavy standalone entry.
  'scene/layoutCtors': 5, // 4->5 in 0.79.0 (layout-critique Cut 3: Layout.computedBoxes/computedPadding/computedGaps public accessors): the 3 new pure-read instance methods (composedBoxes = the memoized #compute result.boxes; computedGaps = the derived main-axis inter-child gaps; computedPadding) landed layoutCtors at EXACTLY 4.00 gz local — a 0-margin trap that CI-tripped 0.79.0-pre.0 (the ~0.16 kB CI gz-delta applies to Node subpath budgets too, not just the browser IIFE). Off-base convenience subpath, ZERO sacred cost (base embed UNCHANGED 38.83/39). Ceiling 5 carries the accessors + the CI-delta + real headroom. // 0.20 no-build layout split (npm subpath `@glissade/scene/layout-ctors`; the budget key matches the dist filename layoutCtors.js): the Yoga-FREE node ctors (Layout/Stack/Row/Column) — they touch the LayoutEngine seam (requireLayoutEngine/setLayoutEngine) only at COMPUTE time, never `import('yoga-layout/load')` at construction. Split off the loader so the @glissade/browser IIFE can expose Stack/Row/Column WITHOUT inlining Yoga's wasm. Standalone bundle inlines the same-package node/displayList helpers it computes through (the @glissade/* external only catches CROSS-package deps); the ctors' own surface is ~1 kB. The "browser IIFE excludes yoga binding" guard verifies the no-inline.
  ...
  'scene/recipes': 9, // 0.63 (capstone): the starter-scaffold factories (lower-third/title-card/stat-reveal/cold-open) — a build-time props→Group fan-out like Grid/Gauge. Ships ONLY on this separate entry (@glissade/scene/recipes), never the base scene index; re-exported onto the @glissade/browser IIFE. The standalone bundle inlines the same-package Group/Rect/Text construction path it builds through (the @glissade/* external only catches CROSS-package deps) + the tiny registerRecipes seam from describe.ts (tree-shaken to just the registry hook), so the measured size is mostly that shared node-construction surface, not the recipes' own ~1 kB. SACRED base embed UNCHANGED (a metafile guard asserts the base scene excludes recipes).
  'scene/diagnostics': 22, // 21->22 in 0.79.0 (layout-critique Cut 3: group→composed-box + LAYOUT_OVERFLOW + settled-hold tracking): composed-box (union of a Group's drawn descendants for containBounds/alignGroups) + LAYOUT_OVERFLOW (per-Layout child ink vs computed slot at the integer-bbox-settled HOLD frame + the per-Layout-child settled-frame tracking). Measured 20.90 -> 20.98 local, but 0.79.0-pre.0 CI-tripped at 21.06 (the ~0.16 kB CI gz-delta applies to Node subpath budgets too, not just the browser IIFE — the 0.10-margin was below it). Off-base convenience/subpath, ZERO sacred cost (base embed UNCHANGED 38.83/39). Ceiling 22 carries the Cut-3 additions + the worst-case CI gz-delta + real headroom (no more 0-2-margin traps). // 19->21 in 0.78.0 (layout-critique Cut 2: MISALIGNED + UNEVEN_SPACING): the explicit-alignGroups sibling-alignment + spacing diagnostics added to critique.ts on this subpath — the settledFrame computation (integer device-bbox adjacent-frame stability per member), the misaligned/unevenSpacing builders + median/validateTolerance/groupLabel helpers + the AlignGroup ingest/fail-loud, plus their actionable message strings (name the offending member + spread + tolerance + fix lever). Measured 18.77 -> 20.25 (+1.48). Off-base convenience/subpath, ZERO sacred cost (base embed UNCHANGED 38.83/39; the "base scene excludes diagnostics" metafile guard covers the additions). Ceiling 21 carries it + the recurring ~0.16-0.48 kB CI-minify delta + real headroom. // 18->19 in 0.66.0 (shared Region-ingest validator): region.ts (validateRegion/RegionError) is imported by critique (safeAreas ingest) and re-exported here, adding ~0.02 kB on top of the 18.00 ceiling critique/assess already sat at. ONE canonical Region boundary shared with the Camera `clear` (motion subpath). Off-base convenience/subpath, ZERO sacred cost (base embed UNCHANGED 38.83/39; the "base scene excludes diagnostics" guard covers region.ts). Ceiling 19 carries it + the recurring ~0.16-0.34 kB CI-minify delta. // 17->18 in 0.63 (assess() capstone, card lLUa_cl-U2iL): assess.ts — the ONE composed VERDICT (validateScene + critique + exportFidelity + diff + certKey, unified/deduped/prioritized + clean-of-fixable + the meaning-preservation veto reads + convergence signature). It COMPOSES the primitives already on this subpath, so its own surface is ~1 kB; ceiling 18 carries it + the recurring ~0.16-0.34 kB CI-minify delta. Off-base convenience/subpath, ZERO sacred cost (base embed UNCHANGED; the "base scene excludes diagnostics" guard now also covers assess.ts). // 15->17 in 0.62 (certKey + canonicalScene, card p9Bn0EjXD-v4): canonicalScene.ts — the ONE canonicalization SHARED by diff + certKey (the load-bearing seam so certKey⟺diff holds by construction) + a pure-TS SHA-256 (browser/node-identical digest; certKey/sceneHash/timelineHash must produce the same content-address in the IIFE and the CLI, and a crypto hash is the honest content-address — a false HIT serves WRONG bytes, so a 256-bit digest not a 32-bit FNV). Measured 14.17 -> 15.78 (+~1.6: the sha256 K-schedule/compression + the canonical scene/timeline serialization; the shared primitives moved OUT of diff.ts, so diff's own size is unchanged). Off-base convenience/subpath, ZERO sacred cost (base embed UNCHANGED 38.67/39; the "base scene excludes diagnostics" guard covers canonicalScene.ts). Ceiling 17 carries the ~0.16-0.34 kB CI-minify delta + headroom. // 13->15 in 0.61 (diff() + exportFidelity, card Hu_Auh6-qZUN): diff.ts (the semantic ChangeSet) + fidelity.ts (the static render-only export scan) added here OFF the base scene index — the "base scene excludes diagnostics" metafile guard now also covers diff.ts + fidelity.ts. Measured 12.82 -> 14.17 (+~1.35: the tree-diff + the render-only-node scan + the ChangeSet canonical sort). Off-base convenience/subpath, ZERO sacred cost (base embed UNCHANGED 38.67/39). // 11->13 in 0.60 (critique(), card mNdrqGB7SZqi): critique.ts — the rendered-diagnostics primitive (OFF_CANVAS/TEXT_OVERFLOW/OCCLUSION) — added here. It runs emitWithIds() + walks the DisplayList, so this subpath now inlines the identity.ts emit instrumentation AND the full scene/node EMIT path (nodes.ts construction + Node.emit) it evaluates through — the @glissade/* external only catches CROSS-package deps, so same-package construction/emit inlines, the standard subpath pattern (matching /identity, /type, /gauge). Measured 9.13 -> 11.82 (+2.69: the emit walk + node-attribution + the 3 codes' predicate/fix-hint logic on the newly-inlined emit path; critique's OWN logic is ~1.5 kB). Ceiling 13 carries that + the recurring ~0.16-0.34 kB CI-minify delta + headroom. Off-base convenience/subpath, ZERO sacred cost (base embed UNCHANGED 38.67/39; the "base scene excludes diagnostics" metafile guard now also covers critique.ts). // 9->11 in 0.59 (fail-loud ground floor, card Hwp8qhV1hGh_): validate.ts (validateScene + resolveAt + instanceProps + Levenshtein) relocated here OFF the base scene index — DIAGNOSTIC surface, never on the evaluate/render hot path — so the SACRED base embed pays ZERO bytes for it (38.67/39, well under ceiling). Measured 7.78 → 9.13 (validateScene inlines the shared Node/scene/text construction path it walks — the @glissade/* external only catches CROSS-package deps, same-package construction inlines, the standard subpath pattern; its own validator/Levenshtein logic is ~1 kB). Ceiling 11 carries that + the recurring ~0.16-0.34 kB CI-minify delta + headroom. Off-base convenience/subpath, ZERO sacred cost; the "base scene excludes diagnostics" metafile guard now also covers validate.ts. // 7->9 in 0.57 (base-budget review): the font-usage collectors + validateSceneFonts (collectTextUsages/collectLocalizedTextUsages/validateSceneFonts) relocated here OFF the base scene index to recover ~0.38 kB gz and keep the SACRED base embed under 39 (it had crept to 38.85, tipping 39.00 in CI). They are CLI/localize/export-path helpers, never on the evaluate/render hot path — the same DEV/CLI rationale as the diff/audit surface. Measured 5.20 → 7.78 (fontUsage inlines the shared Group/Text/measurement construction path it walks — the @glissade/* external only catches CROSS-package deps, so same-package construction inlines, the standard subpath pattern). Ceiling 9 carries that + the ~0.16-0.34 kB CI-minify delta + headroom. Off-base convenience/subpath, ZERO sacred cost; the "base scene excludes diagnostics" metafile guard now also covers fontUsage.ts. // 0.20: the §3.3 DEV/CLI determinism-diagnostic surface — diffDisplayLists / formatDisplayDiff / serializeDisplayList / parseDisplaySnapshot, auditCacheCold. Side-effect-free, NEVER reached by evaluate(); relocated off the base scene index in the 0.20 budget review onto this tree-shakeable subpath. DEBUG-ONLY (tokenHighlight, a PRODUCTION render component, was split back out onto scene/tokens by the ai-training finding — it no longer rides this debug subpath). (`collapseReplacer` — the byte-preserving §3.5 cacheKey replacer — is the one piece on the render path; it stays in collapseReplacer.ts on the base index and is re-exported here.) The standalone bundle inlines the same-package displayList/node helpers it compiles through (the @glissade/* external only catches CROSS-package deps), so the measured size is mostly that shared path; base scene stays diagnostics-free (asserted by the metafile guard below).
  ...
  browser: 94, // 93->94 in 0.79.0 (layout-critique Cut 3: group→composed-box + LAYOUT_OVERFLOW on the IIFE): the composed-box resolution (union of a Group's drawn descendants for containBounds/alignGroups members) + the LAYOUT_OVERFLOW probe (ink bbox vs computed Yoga slot) land on the inlined /diagnostics construction path. Measured 92.19 -> 92.96 (+0.77; partly offset by deduping inline rounded-box literals through the shared intBox helper). The SACRED base embed is UNCHANGED (38.83/39; critique rides the tree-shakeable /diagnostics subpath). 92.96 local sat only 0.04 kB under 93 — WELL below the recurring ~0.16-0.48 kB CI-minify delta (~93.06-93.44 in CI = a near-certain boundary red-fail, per the 0.65.0-pre.1/0.67.0-pre.0 traps) — so lifted to 94 to carry the feature delta + that worst-case CI delta + real headroom. The Layout computed-accessors (computedBoxes/Padding/Gaps) are instance methods NOT re-exported onto the IIFE, so they add ZERO here (they ride scene/layoutCtors 4.00/4). // 91->93 in 0.78.0 (layout-critique Cut 2: MISALIGNED + UNEVEN_SPACING + alignGroups on the IIFE): the sibling-alignment + spacing diagnostics re-exported onto window.glissade off @glissade/scene/diagnostics so the no-build author reaches critique's alignGroups/MISALIGNED/UNEVEN_SPACING against the IIFE (its primary audience). The IIFE inlines the /diagnostics construction path, so critique's Cut-2 feature code (settledFrame integer-bbox-stability + the two diagnostic builders + median/validateTolerance/groupLabel + the AlignGroup ingest + the actionable message strings) lands here. Measured 90.26 -> 91.98 (+1.72). The SACRED base embed is UNCHANGED (38.83/39; critique rides the tree-shakeable /diagnostics subpath, asserted by the "base scene excludes diagnostics" metafile guard). 91.98 local clips the 91 ceiling; lifted to 93 (not 92) so the ~0.16-0.48 kB CI-minify delta can't clip it (a +1 to 92 would leave only 0.02 headroom = a near-certain CI red-fail per the 0.65.0-pre.1/0.67.0-pre.0 boundary traps). // 90->91 in 0.77.0 (OUT_OF_BOUNDS critique + containBounds on the IIFE): the keep-WITHIN-box diagnostic (the inverse of CAPTION_COLLISION) re-exported onto window.glissade off @glissade/scene/diagnostics so the no-build author reaches critique's containBounds/OUT_OF_BOUNDS against the IIFE (its primary audience — a diagnostic absent from window.glissade is unusable to that consumer). Measured 89.65 -> 90.03 (+0.38: the outOfBoundsDiagnostic builder + the containBounds ingest/accumulation/check on the inlined /diagnostics construction path). The SACRED base embed is UNCHANGED (38.83/39; critique rides the tree-shakeable /diagnostics subpath, asserted by the "base scene excludes diagnostics" metafile guard). 89.65 local already sat only 0.35 kB under 90 — below the recurring ~0.16-0.48 kB CI-minify delta — so 90.03 clips the 90 ceiling; lifted to 91 to carry the feature delta + that worst-case CI delta + real headroom. // 89->90 in 0.68.0 (caption-split: splitToFit + TextFitError on the IIFE): the measured band-aware caption splitter re-exported onto window.glissade off @glissade/scene/type so the no-build author reaches splitToFit + catches `e instanceof glissade.TextFitError` against the IIFE (its primary audience — a tool absent from window.glissade is unusable to that consumer). Measured 87.66 -> 88.82 (+1.16: captionSplit.ts's boundary-segmentation + recursive band-pack logic + the TextFitError class, inlined here off the /type construction path). The SACRED base embed is UNCHANGED (38.83/39; captionSplit rides /type, guarded off the base). NOTE: 88.82 local left only 0.18 headroom under 89 — below the recurring ~0.16-0.48 kB CI-minify delta (0.67.0-pre.0 red-failed at exactly this trap, CI 0.48 heavier) — so the ceiling is lifted to 90 to carry the worst-case delta + real headroom. // 88->89 in 0.67.0 (measurer-fail-loud: MeasurerRequiredError instanceof-catchable off the IIFE): the fail-loud text-geometry surface re-exports MeasurerRequiredError onto window.glissade so the no-build author can `catch (e) { if (e instanceof glissade.MeasurerRequiredError) ... }` against the IIFE (its primary audience — the resolveMeasurer throw is unusable to that consumer if the error type isn't reachable). The dead warn-once bookkeeping removal offset the base scene threading (the SACRED base embed actually SHRANK 38.83->38.78/39 — a throw is not a render change, and the removed warnIfEstimating/__resetEstimateWarnings machinery outweighs the flag plumbing). NOTE: the 0.67.0-pre.0 release RED-FAILED at CI 88.09 while local measured 87.61 — the recurring CI-heavier delta ran ~0.48 kB HERE, the TOP of the documented ~0.16-0.34 band, clipping the 88 ceiling (only 0.39 local headroom). Ceiling lifted to 89 to carry that worst-case delta + real headroom. // 87->88 in 0.66.0 (Camera node-framing on window.glissade): centerOn/clear ride the already-exposed `camera` factory (its AUTHOR-facing surface must be IIFE-reachable day-one — the no-build design agent's primary audience), plus the shared validateRegion/RegionError re-exported off @glissade/scene/diagnostics. Measured 87.09/87 (+~0.24: the world-focal + derived-clear + resolvedCenter + region-validator surface on the inlined /motion + /diagnostics construction path). The SACRED base embed is UNCHANGED (38.83/39 — centerOn/clear live on the /motion subpath + the region validator on /diagnostics, never the base scene index). Ceiling lifted to 88 for that + the recurring ~0.16-0.34 kB CI-minify delta + headroom. // 86->87 in 0.65.0 (gs-player poster+persist, card cRf45pkAwcNn): the <gs-player> poster/poster-t/persist attribute methods ship in @glissade/element, which is INLINED whole into the single-file IIFE (§4.4 embed path includes element) — poster is a NO-BUILD embed feature whose PRIMARY audience IS this bundle (a first-frame still for pre-play / prefers-reduced-motion rest state / no-JS / OG-thumbnail). renderToDataURL/snapshotCanvas were already on window.glissade since 0.19, so the growth is the element's OWN #refreshPoster/#buildPoster/#syncPersist methods (element 2.22 -> 2.24 kB), which tipped a bundle sitting EXACTLY at the 86.00 ceiling after 0.63. The SACRED base embed is UNCHANGED (38.82/39 — poster/persist are player/element-side, never touch evaluate or the base scene index). Ceiling lifted to 87 for that + the recurring ~0.16-0.34 kB CI-minify delta (the 0.65.0-pre.1 release RED-FAILED at CI 86.01 while local measured ~85.85 — the documented CI-heavier trap) + headroom. // 83->86 in 0.63 (assess + recipe capstone on window.glissade, card lLUa_cl-U2iL): assess (kind:'tool' — the ONE composed VERDICT) + recipe (kind:'tool' — the clean-by-construction starter scaffolds) re-exported onto window.glissade off @glissade/scene/diagnostics + @glissade/scene/recipes so the no-build agent runs the author→assess→auto-fix-geometry→re-assess loop against the IIFE (its PRIMARY audience). assess composes primitives already on the bundle (critique/diff/certKey/exportFidelity) so it adds mostly its own ~1 kB; recipe adds the props→Group factories on the already-inlined node-construction path. Ceiling lifted to 86 for that + the recurring ~0.16-0.34 kB CI-minify delta + headroom. The SACRED base embed is UNCHANGED (the "base scene excludes diagnostics/recipes" guards cover assess.ts + recipes.ts). // 82->83 in 0.62 (certKey on window.glissade, card p9Bn0EjXD-v4): certKey (kind:'tool') + sceneHash/timelineHash re-exported onto window.glissade off the @glissade/scene/diagnostics subpath so the no-build author queries "will this render be a cache hit?" against the IIFE BEFORE spending the render farm. Rides the pure-TS SHA-256 + canonicalScene (the SAME canonicalization diff uses). Measured 79.58 -> 81.18 (+1.60: sha256's compression loop + canonicalScene serialization on the inlined /diagnostics construction path). The SACRED base embed is UNCHANGED (38.67/39; the "base scene excludes diagnostics" guard covers canonicalScene.ts). Ceiling lifted to 83 for the ~0.16-0.34 kB CI-minify delta + headroom. // 79->82 in 0.61 (diff() + exportFidelity, card Hu_Auh6-qZUN): diff (a ChangeSet TOOL) + exportFidelity (a static render-only export DIAGNOSTIC) re-exported onto window.glissade off the @glissade/scene/diagnostics subpath so the no-build author reaches the author->patch->diff->verify loop + the export-fidelity scan against the IIFE. Measured 76.86 -> 79.58 (+2.72: diff's tree/track/DisplayList diff + fidelity's node scan + the ChangeSet types on the inlined construction path). The SACRED base embed is UNCHANGED (38.67/39; the "base scene excludes diagnostics" guard covers diff.ts + fidelity.ts). Ceiling lifted to 82 for the ~0.16-0.34 kB CI-minify delta + headroom. // 75->79 in 0.60 (critique(), card mNdrqGB7SZqi): critique + emitWithIds re-exported onto window.glissade so the no-build author reaches the author->render->critique->self-fix loop against the IIFE (its primary audience — a tool absent from window.glissade is unusable to that consumer). emitWithIds (the node-id-stream producer critique needs, previously only on the separate glissade-dom.browser.js) rides here too. Measured 73.48 -> 76.67 (+3.19: critique's own OFF_CANVAS/TEXT_OVERFLOW/OCCLUSION logic + emitWithIds's instrumented builder + the emit/attribution path they reach through — on the tree-shakeable /diagnostics + /identity subpaths but inlined here). The SACRED base embed is UNCHANGED (38.67/39; the "base scene excludes diagnostics/identity" metafile guards assert critique.ts + identity.ts stay off the base). Ceiling lifted to 79 for the recurring ~0.16-0.34 kB CI-minify delta (76.67 local -> ~77 CI) + real headroom. // 73->75 in 0.59 (fail-loud ground floor, card Hwp8qhV1hGh_): the AUTHOR-facing 0.59 diagnostics subset — validateScene / resolveAt / instanceProps / DIAGNOSTIC_SCHEMA_VERSION (+ MeasurerRequiredError) — re-exported onto window.glissade off the tree-shakeable @glissade/scene/diagnostics subpath so the no-build author reaches the eager fail-loud validator + truthful read primitive against the IIFE (its primary audience — a tool absent from window.glissade is unusable to that consumer). The HEAVY determinism/diff + fontUsage tooling on the same subpath (diffDisplayLists/serializeDisplayList/auditCacheCold/validateSceneFonts) stays ESM-only, NOT on the IIFE. Measured 72.37 → 73.48 (+1.11: validateScene inlines the Levenshtein + resolveAt + instanceProps + the Node/scene/text construction path they walk through — the @glissade/* external only catches CROSS-package deps, so same-package construction inlines, matching the /diagnostics 7.78→9.13 subpath delta). The SACRED base embed is UNCHANGED (38.67/39; the "base scene excludes diagnostics" metafile guard asserts validate.ts stays off the base). Ceiling lifted to 75 for the recurring ~0.16–0.34 kB CI-minify delta (73.48 local → ~73.8 CI) + real headroom. // 69->73 in 0.57 (Particles/Emitters): particles/drift/sparks/dispense re-exported onto window.glissade so the no-build design agent reaches the seeded baked emitter + presets against the IIFE (its primary audience — a feature absent from window.glissade is unusable to that consumer), plus the four describe().helpers entries. Measured 67.32 → 71.58 (+4.26: the emitter/presets surface + the each.ts + Circle/Text construction path they reach through — modules only partly inlined before — on the tree-shakeable /motion subpath but inlined here). The SACRED base embed is UNCHANGED (38.85/39; the "base scene excludes motion" guard asserts /motion — now incl particles — stays off the base). Ceiling lifted to 73 for the recurring ~0.16–0.32 kB CI-minify delta (the 0.55 note: CI ran 0.32 heavier here once) + real headroom. // 67->69 in 0.56 (kinetic type presets): typeOn/revealWords/revealLines/emphasizeWords re-exported onto window.glissade so the no-build kinetic-typography author reaches the one-call type sugar against the IIFE (its primary audience — a feature absent from window.glissade is unusable to that consumer). Measured 65.79 → 67.32 (+1.53: typeOn inlines typewriter.ts + textCursor.ts's closed-form caret draw — modules NOT previously pulled onto this bundle — plus the four presets' stagger-composition surface and their describe().helpers strings, on the tree-shakeable /type subpath but inlined here). The SACRED base embed is UNCHANGED (38.84/39; the "base scene excludes splitText" guard asserts /type stays off the base). Ceiling lifted to 69 for the recurring ~0.16–0.32 kB CI-minify delta (the 0.55 note: CI ran 0.32 heavier here once) + real headroom. // 64->67 in 0.55 (Camera rig): camera / shake (+ CameraError/shakeOffset) re-exported onto window.glissade so the no-build design agent reaches cinematic camera moves against the IIFE (its primary audience — a feature absent from window.glissade is unusable to it), plus valueNoise (via `export * from '@glissade/core'`) and the three describe().helpers entries. Measured 65.79 local (+~1.5: the camera/shake surface on the inlined /motion construction path + the helper strings). The SACRED base embed is UNCHANGED (38.83/39; a metafile guard asserts the base scene excludes camera/shake/motion). NOTE: an initial 66 (from 65.77 local) RED-FAILED the release — CI minified 66.11 (the recurring CI-heavier delta ran ~0.32 kB here, LARGER than the usual ~0.16 estimate), so the ceiling is 67 to carry the delta + real headroom. GOTCHA: the pre-release local `check:size` SKIPS browser unless `pnpm build:browser` ran first — build the IIFE before check:size to see this bundle. §4.4 single-file IIFE: the WHOLE embed path INLINED (core+scene+canvas2d+player+element) PLUS the @glissade/core/clips tier (presence/each/morph/clip + stdlib) for window.glissade discoverability — measured from the prebuilt dist/glissade.browser.js (42.3 kB measured w/ clips + headroom; was 39.3 kB before clips). 45→46 in 0.18 pre.6 for describe() construction-completeness: the richer API manifest (construction props + the curated Layout family schema + the assets-shape createScene string + the negative-space prop entries) is a static data table re-exported onto window.glissade for AI discoverability — it is NOT on the base embed path (describe stays tree-shaken off the base scene index; base embed measured UNCHANGED at 38.15 kB), only this convenience bundle inlines it (45.12 kB measured). 46→47 in 0.19 to expose renderToDataURL/snapshotCanvas on window.glissade (the +0.36 kB screenshot DX seam): the Claude-Design no-build consumer works ONLY against the IIFE, so a feature absent from window.glissade is unusable to it — the snapshot helper's primary audience IS this bundle. This is the CONVENIENCE bundle, NOT the sacred base embed (which stays tree-shaken-lean at 38.6/39); browser measures ~45.96. 47→48 in 0.20 for the no-build authoring + DX features re-exported onto window.glissade — Stack/Row/Column + loadYogaLayoutEngine (Yoga itself stays an async import, NOT inlined), Grid (the scene/grid track resolver), the describe() helpers section, the friendlier construction-prop bind message, and static variable-font passthrough — all features the no-build Claude-Design consumer works against window.glissade for. The SACRED base embed actually SHRANK this milestone (38.79 → 35.55 via the 0.20 budget-review relocations), so the convenience bundle's growth is decoupled from the embed budget. 48→49 in 0.20.1 for the fail-loud node-constructor guard (Node.checkProps + acceptedConstructionKeys + NodeConstructionError + the per-leaf new.target calls) — it ships in scene, so it rides this bundle; +the corrected/longer splitText describe() usage string + the splitText by-guard. The guard is a real safety feature (rejects silently-dropped ctor props like `new Rect({ size })`, the shipped footgun all three canary seats confirmed). The SACRED base embed stays healthy (35.62 → 36.00/39, +0.38 for the guard's ctor-path code — well under ceiling); only this convenience bundle needed the bump. 49→50 in 0.23 for scene.measureWrappedText on window.glissade (node-free wrapped-text sizing — the design-agent's #1 DOM follow-up; its primary audience is the no-build IIFE consumer) + its describe() helper entry: measured 48.43 → 48.90 local, but CI minifies ~0.16 kB HEAVIER than local (the recurring delta — see the 0.19/0.20.1 notes), so 48.90 local clipped the 49 ceiling at ~49.06 in CI and red-failed the release. The 0.23 feature-minor wave (variable-font axis animation, the onboarding-harness helpers) also rides this convenience bundle; the SACRED base embed stays lean (36.10 → 36.23/39). 50→53 in 0.24 (onboarding, card 8jQ9rNqStGDL, Tyler-approved): the runnable example corpus (@glissade/scene/examples) is registered onto window.glissade so `glissade.describe({ examples: true })` surfaces a copy-pasteable, doctest-verified snippet per node/builder/helper — THE no-build agent's primary onboarding fix (a cold agent's worst time-sink is stale examples; here the canonical snippet literally can't drift, it's run in CI). Measured 49.64 → 51.40 (+1.76: the corpus's own surface + the layoutCtors/motionPath/path construction bits it exercises that weren't already inlined). A side-effect import (examples.js is package.json-sideEffectful so it isn't tree-shaken); the corpus rides ONLY this convenience bundle — the SACRED base embed stays UNCHANGED at 36.74/39 (a metafile guard asserts the base scene excludes examples). Headroom to 53 for the ~0.16 kB CI-minify delta + the deferred fail-loud-sweep additions. Not measured by the standalone loop below (that externalizes @glissade/*, which would leave an empty shell) — see the dedicated block after the loop. 53→56 in 0.26 (motion-craft quick-wins wave): the wave re-exports new authoring/motion helpers onto window.glissade for the no-build consumer — orientToPath/lookAt (rotation drivers, off the base via the /motion subpath but inlined here) and retime (the pure key-time transform: speed/reverse/pingpong). Measured 52.20 (orient) → 52.99 (retime); the ~0.79 kB is those helpers' own surface plus the shared construction path they reach through. 53 had ~0.01 kB headroom after retime, so the ceiling is lifted to 56 to carry the rest of the wave (echo/trails, responsive format) + the recurring ~0.16 kB CI-minify delta. The SACRED base embed stays lean (36.74 → 37.46/39; retime rides the core index alongside stagger/springTo — orient does NOT touch the base, it's on the tree-shakeable /motion subpath, asserted by the metafile guard). 56→58 in 0.32 (the data-motion stack): Chart + the scale factories (linearScale/logScale/bandScale/colorRamp) are re-exported onto window.glissade so the no-build data-viz author can bind a table → animated bar chart against the IIFE (its primary audience — a feature absent from window.glissade is unusable to that consumer). Measured 55.99 → 56.31 (+0.32: Chart's layout + the scales' math + the Rect/Group construction path they reach through, on the tree-shakeable /chart subpath but inlined here). The SACRED base embed is UNCHANGED (37.46 → 38.00/39; a metafile guard asserts the base scene excludes chart); ceiling lifted to 58 for the ~0.16 kB CI-minify delta + headroom. 58->60 in 0.35: fitText/fitTextSize/fitTextGroup re-exported onto window.glissade so the no-build author reaches shrink-to-fit (same /type subpath + measurer plumbing as splitText); measured 58.37 (fitText's binary-search + FontSpec logic on the inlined text-construction path). box-valign rides the base scene index (Text draw path), so it's in the base embed not this bundle; the SACRED base embed is 38.83/39. Ceiling 60 carries the delta + headroom. 60->62 in 0.38 (radial data-viz): Gauge + Meter re-exported onto window.glissade so the no-build author reaches value→needle / scripted-needle gauges (its primary audience — a feature absent from window.glissade is unusable to that consumer). Measured 59.15 → 60.17 (+1.02: Gauge's arc/needle geometry + the Path/Rect/Text/Circle/Group construction path it reaches through — more node types than Chart, on the tree-shakeable /gauge subpath but inlined here). The SACRED base embed is UNCHANGED (38.83/39; a metafile guard asserts the base scene excludes gauge); ceiling lifted to 62 for the ~0.16 kB CI-minify delta (60.17 local → ~60.33 CI) + headroom. 62->64 in 0.40 (Expr): exprTrack + compileExpr (@glissade/core/expr formula evaluator) re-exported onto window.glissade + the exprTrack describe().helpers entry; measured 61.00->62.72 (+1.72: tokenizer/parser + describe string, /expr subpath inlined here). SACRED base embed UNCHANGED 39.00/39 (evaluator off base, only the sampler seam rides it — metafile guard asserts base core excludes expr); ceiling 64 for the delta + headroom.
};
...
const diagInBase = Object.keys(sceneIndex.metafile.inputs).filter((i) =>
  /scene\/(src|dist)\/(displayDiff|cacheColdAudit|diagnostics|fontUsage|validate|critique|diff|fidelity|assess)\./.test(i),
);
...
// 0.63 guard (capstone): the base scene bundle must NOT pull in the starter-recipe
// factories (recipes.ts) — they're the separately-budgeted @glissade/scene/recipes
// entry (a build-time props→Group fan-out like Grid/Gauge, re-exported onto the
// browser IIFE). The base index never imports them, so assert via the metafile that
// recipes.ts stays off the base graph — the examples/grid exclusion shape.
const recipesInBase = Object.keys(sceneIndex.metafile.inputs).filter((i) => /scene\/(src|dist)\/recipes\./.test(i));
const recipesOk = recipesInBase.length === 0;
if (!recipesOk) failed = true;
console.log(`${recipesOk ? 'ok  ' : 'FAIL'} base scene excludes recipes${recipesOk ? '' : ` (leaked: ${recipesInBase.join(', ')})`}`);
